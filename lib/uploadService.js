...
export const addQuestion = async (question) => {
  try {
    // If question has a code field, use it as the document ID
    if (question.code) {
      ...
      // If no code field, let Firestore auto-generate an ID
...
export const updateTestBatch = async (testBatchId, quizId, quizData) => {
  try {
    ...
    if (existingExamIndex === -1) {
      ...
      console.log(
        `Created new exam entry for "${quizData.title}" with ${quizData.language} version`
      );
    } else {
      ...
      if (languageIndex === -1) {
        ...
        console.log(
          `Added ${quizData.language} version to existing quiz "${quizData.title}"`
        );
      } else {
        existingExam.quizIds[languageIndex].quizId = quizId;
        console.log(
          `Updated ${quizData.language} version of quiz "${quizData.title}"`
        );
      }
      ...
      console.log(
        `Updated primary quiz ${existingExam.primaryQuizId} with language versions`
      );
    }
