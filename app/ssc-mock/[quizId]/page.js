    ...
    const fetchQuizData = async () => {
      try {
        ...
        if (showPreviousSubmission) {
          ...
          if (user) {
            console.log("User authenticated, fetching previous submission");
            try {
              const previousSubmission = await getUserSubmission(
                user.uid,
                params.quizId
              );
              console.log("Previous submission fetched:", previousSubmission);
  ...
  const handleSubmitQuiz = async () => {
    ...
    const userSubmission = quizData.sections.map((section) => ({
      ...
      questions: section.questions.map((question) => ({
        ...
        isCorrect:
          String(question.selectedOption) === String(question.correctAnswer),
      })),
    }));
    ...
    try {
      ...
      console.error("Error updating user quiz data and submission:", error);
    }
  };
