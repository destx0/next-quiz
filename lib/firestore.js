...
export const getQuizWithQuestions = async (quizId) => {
  try {
    // Get the quiz data from fullQuizzes instead of quizzes
...
export const updateQuestion = async (questionId, questionData) => {
  try {
    ...
    // Find and update in fullQuizzes collection
    const fullQuizzesRef = collection(db, "fullQuizzes");
    const q = query(fullQuizzesRef, where("sections", "array-contains", questionId));
    const querySnapshot = await getDocs(q);
    
    // Update each quiz that contains this question
    const updatePromises = querySnapshot.docs.map(async (quizDoc) => {
      const quizData = quizDoc.data();
      const updatedSections = quizData.sections.map(section => ({
        ...section,
        questions: section.questions.map(q => 
          q.id === questionId ? { id: questionId, ...questionData } : q
        )
      }));

      await updateDoc(doc(db, "fullQuizzes", quizDoc.id), {
        sections: updatedSections
      });
    });

    await Promise.all(updatePromises);
    console.log("Question updated successfully in both collections");
  } catch (e) {
