...
export const createOrganizerStructure = async () => {
  try {
    // Create SSC CGL document
    const sscRef = doc(db, "organizer", "ssc_cgl");
    const sscStructure = {
      name: "SSC CGL",
      pyqs: "/testBatches/7YIxWJRTqXyY8SHk7fUH",
      full_mock: "/testBatches/7YIxWJRTqXyY8SHk7fUH",
      topic_wise: {},
    };
    await setDoc(sscRef, sscStructure);

    // Create RRB document
    const rrbRef = doc(db, "organizer", "rrb");
    const rrbStructure = {
      name: "RRB",
      pyqs: "/testBatches/7YIxWJRTqXyY8SHk7fUH",
      full_mock: "/testBatches/7YIxWJRTqXyY8SHk7fUH",
    ...
    await setDoc(rrbRef, rrbStructure);

    return { ssc_cgl: sscStructure, rrb: rrbStructure };
  } catch (error) {
    console.error("Error creating organizer structure:", error);
    throw error;
  }
};
